...
| [#284](../../issues/284) | a11y wave 2 — migrate remaining ~27 single-instance modals             | ✅ 24 of 26 migrated (PR pending) — `TransitionPanel` (workflow-canvas.tsx) and the access-denied overlay (record-detail.tsx) deferred per the issue's own flag. Also fixed a pre-existing bug found along the way: `packages/ui`'s `DialogContent` always rendered its own close button even when a modal supplied its own, producing two close affordances (already live via `transition-modal.tsx`, wave 1) — added an opt-out `showCloseButton` prop, used on every migrated modal that has its own close control. |
