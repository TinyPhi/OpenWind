...
Tickets and comments support file uploads via `@platform/files` — tenant-scoped local-disk storage with async ClamAV malware scanning (PR #340; replaced an earlier S3/MinIO presigned-URL design), never a public path, gated by the same record-level access check above and a per-tenant storage quota.
...
Every file uploaded is stored on local disk under a tenant-scoped path (`{tenantId}/{moduleId}/{entityId}/{filename}`, bind-mounted via `FILES_STORAGE_PATH_HOST`), scanned asynchronously by ClamAV before being marked downloadable. File metadata is in Postgres, subject to RLS. A file belonging to tenant A can never be accessed by tenant B even if the UUID is guessed, because access is validated against tenant ownership before the file is served — never a public path.
...
│   └── admin-ui/     # Refine + shadcn/ui — serves admin, agent, AND
│                     #   customer/portal views (RBAC-controlled); there
│                     #   is no separate portal app (apps/portal source
│                     #   was removed in PR #211, stub directory only)
├── packages/
│   ├── entity-engine/
│   ├── workflow-engine/
│   ├── automation-engine/
...
Development: a single `docker compose up -d` starts the default stack — Postgres, PgBouncer, Redis, OpenBao, Zitadel, ClamAV, the API server, workers, and admin UI. Novu (notifications) and the observability stack (Prometheus/Grafana/Alertmanager) are opt-in via `--profile notifications`/`--profile observability`. There is no MinIO — file storage moved to local disk + ClamAV scanning (PR #340). Every developer has a fully functioning local environment within minutes via `pnpm bootstrap` — see [`docs/local-setup.md`](local-setup.md).
...
| `@platform/files`             | Local-disk storage, async ClamAV scanning, metadata    |
