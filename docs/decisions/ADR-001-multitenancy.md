...
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| `tenants`                       | **Excluded** by migration 0113's least-privilege pass                                                                     | No tenant column, so no RLS; org → tenant is answered by `tenant_for_org()` (0114) |
| `entity_types`                  | All columns                                                                                                               | No sensitive data                                                                  |
| `entity_fields`                 | **Excluded** by migration 0113's least-privilege pass                                                                     | No provisioned dataset reads it                                                    |
...
| `entity_relations`              | **Excluded** by migration 0113's least-privilege pass                                                                     | No provisioned dataset reads it                                                    |
| `workflows` / `workflow_states` | All columns                                                                                                               | Config only, no PII; names and `is_terminal` for charts                            |
| `workflow_transitions`          | **Excluded** by migration 0113's least-privilege pass                                                                     | No provisioned dataset reads it                                                    |
...
| `automation_rules`              | **Excluded** by migration 0113's least-privilege pass                                                                     | No provisioned dataset reads it; `actions` JSONB may contain webhook URLs/API keys |
| `automation_executions`         | **Excluded** by migration 0113's least-privilege pass                                                                     | No provisioned dataset reads it                                                    |
