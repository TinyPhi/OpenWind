...
**Status:** Proposed (documents already-shipped work — see Context for why this is retroactive).  
**Date:** 2026-08-19.  
...
| ID   | Question                                                                                                                                                               | Notes                                                                                                                                                                                                                                                                                                 |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OQ-1 | Should CDN mirroring / an origin allowlist for plugin remotes (issue #6) be formally waived for the first-party-only trust tier, or built anyway?                      | No decision exists either way today — see Decision #10 and Consequences.                                                                                                                                                                                                                              |
...
| OQ-3 | Is forward-only plugin migration an acceptable permanent posture, or does it need up/down pairs before third-party plugins are ever considered?                        | Not resolved; flagged as a Deferred Decision trigger above.                                                                                                                                                                                                                                           |
| OQ-4 | Does `roadmap-tracker.md`'s 3B "100% ... all 3 phases" entry need a caveat noting route/hook/job loading isn't built, or was that always outside 3B's committed scope? | Raised during this ADR's adversarial review (2026-08-19) — the ADR's own Decision #1 language ("a plugin can run arbitrary backend code") reads as a present-tense capability claim next to that 100%, when the actual live capability is migrations-only. Needs a maintainer call, not an agent one. |
...
2. Resolve OQ-2 via issue #433.
3. Decide and log OQ-1 explicitly — even if the answer is "not needed while first-party-only,"
   write it down so it is a decision, not a gap.
4. Resolve OQ-4 — reconcile `roadmap-tracker.md`'s 3B status line against the route/hook/job loading
   gap, one way or the other.
5. Once accepted, cross-link this ADR from `docs/specs/plugin-system.md`'s header and from issue
   #6's plugin-marketplace-security checklist (already partially reconciled in that issue's
   2026-08-19 comment).
