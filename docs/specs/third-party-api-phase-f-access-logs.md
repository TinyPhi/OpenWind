...
| T2  | Admin route `GET /api/admin/third-party-access-logs` — wraps the existing `queryAuditLog`, joins/resolves `applicationName` from `api_keys` by actorId, applies T1's outcome classification, adds the `outcome` filter (queryAuditLog itself has no such filter yet — needs a small, additive extension to `@platform/audit`, a shared package; keep it purely additive so other callers are unaffected) | 1     | todo   | T1             |
