...
| retention    | Indefinite, no auto-delete (2026-08-14 team decision: audit-trail-over-erasure default, revisitable later if a concrete compliance need arises) — this phase must NOT add a retention/cleanup job                                                                                                                                                                                                                            |
...
- Log retention is indefinite by design (2026-08-14 decision) — never add a cron/cleanup job that
  deletes `admin_audit_log` rows as part of this phase. If retention is revisited later, that's a
  separate, explicit decision with its own ADR amendment, not a side effect of this phase's work.
...
| T3  | Admin-ui screen: filterable table (application/person/ticket/date-range/outcome), matching the existing admin screen conventions                                                                                                                                                                              | 1     | todo   | T2             |
