...
a different route). Provisioning and the API must read the same identifiers from the same
configuration source, so that a clone-and-run deployment agrees with itself. **T12b** closes it
...
| T12b | **remove the hardcoded dashboard identifiers (R17).** `apps/api/src/routes/reporting/dashboard-ids.ts` holds four literal UUIDs; provisioning generates its own, so a fresh clone mints a valid pass whose every dashboard call 404s. Provisioning (`bootstrap.py`) and the mint endpoint must read the same identifiers from the same configuration, via `@platform/config` like every other setting — no UUID literals in application source                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | T12                    |
