...
**D6 — KPI cards are native Superset charts, in the dashboard's first row, not a custom React
layer outside it** (decided 2026-09-17). An earlier pass built the 5 KPI cards as a separate
`KpiCard` React component (`apps/admin-ui/src/pages/reporting-kpi-card.tsx`), calling Superset's
`/api/v1/chart/data` REST endpoint directly from the browser with a guest token, with the
backing charts marked `hidden: true` in `tiles.yaml` so the iframe never shows them. That is
reverted: the 4 tenant-tab KPI tiles (Total Tickets, Overdue Now, Unassigned Tickets, Median
Time to First Response) are unhidden and placed as the dashboard's first row, one row of four,
rendered by Superset itself inside the same iframe as every other tile.

Rationale: the custom-card path was a second, parallel way to reach tenant data — a direct
browser→Superset REST call bearing a guest token, separate from the iframe's own request path —
which is one more surface to keep in sync with every RLS/filter change (P1, P1b) for no benefit
once Superset can render the same chart natively. Folding it into the dashboard removes that
duplicate path entirely; `resolveChartId()` (`superset-client.ts`) and `KPI_CHART_NAMES`
(`guest-token.ts`) — both built only to support the custom cards' REST calls — are removed as
dead code in the same change.

Two per-card features carry over, native-first instead of custom-built:

- **ⓘ hint** — Superset's own chart `description` field (rendered as an info icon in the chart
  header) replaces the custom hover tooltip in `reporting-kpi-card.tsx:239-297`. Each KPI tile in
  `tiles.yaml` gets a `description:` value; none is set today.
- **Per-card date filter** — a native Superset time filter, one per KPI tile, each scoped via
  `chartsInScope` to that tile alone. This is new ground for this repo: every native filter
  provisioned so far (`bootstrap.py`'s `apply_filters`) is a single time filter scoped to _every_
  chart on the dashboard (`tiles.yaml`'s existing `Date Range` filters). Four filters, each
  scoped to one chart, has no precedent to copy — see OQ-16. `select`/`range` filter types remain
  off-limits per the existing isolation comment (`tiles.yaml:734-753`); `time` filters are exempt
  because they carry their own range and issue no options-query against the database, so they do
  not hit the RLS gap that comment describes.

Standalone Stage 2, once built, inherits this row for free — it is the same dashboard, just
reached without the embed iframe, so no separate KPI-row work is needed there.

**D7 — Superset re-pinned 6.0.0 → 6.1.0** (decided 2026-09-17). Found while making the embedded
dashboard follow OpenWind's own light/dark toggle (a separate, pre-existing ask, not part of D6):
`embedded.setThemeMode()` — the SDK's documented mechanism for this — was called successfully, with
no error, and produced **no visual change at all** on 6.0.0. Verified two ways before concluding it
was a version gap and not a config bug: (1) a fresh embed mounted with `themeMode: 'dark'` from the
very start (no live switch involved) still rendered light, and (2) 6.1.0's own changelog lists
`setThemeMode()` as _newly functional_ (#36125) — 6.0.0 shipped the API surface with no working
implementation for embedded/guest viewers. Confirmed after the version bump: the same live call now
switches the dashboard's text color to the exact configured `THEME_DARK` token.

Checked before re-pinning: no CVEs published against 6.1.0; `UPDATING.md` documents no breaking
change to `DB_CONNECTION_MUTATOR`, guest tokens, RLS, `GUEST_ROLE_NAME`, or native filters between
6.0.x and 6.1.0. One regression found by testing anyway, not by the changelog: 6.1.0 tightened
`GET /api/v1/me/roles/` (permission `can_read on CurrentUserRestApi`) in a way that broke the guest
role's own embedded-page boot sequence — 403, blank dashboard, unrelated to theming. Fixed by adding
that permission to `EMBEDDED_VIEWER_PERMISSIONS` in `bootstrap.py`. Both `docker/superset/
Dockerfile`'s `FROM` and `docker-compose.yml`'s `openwind-superset:` image tag move in lockstep per
existing convention (§C).
...
### P1c — `security_invoker` and the P2 grant are mutually exclusive as written

Found 2026-09-14 while sequencing T3b, and it is the reason OQ-13 could not simply be closed
"register the masked view". Live-tested against `ow-database` in rolled-back transactions.

The fix above requires the **caller** to hold read grants on what the view selects. The view's body
calls `jsonb_each(we.metadata)` to build its redacted output — so the caller needs a grant on
`metadata`, which is exactly the column P2 withholds:

```
-- security_invoker = true, grants per the P2 allowlist (raw metadata withheld):
SELECT count(*) FROM workflow_events_masked;
ERROR:  permission denied for table workflow_events

-- identical, with metadata added to the grant:
SELECT count(*) FROM workflow_events_masked;   -->  0 rows
--   (0 is correct: app.tenant_id was unset, so RLS failed closed)
```

So the view either errors for the reporting role, or `metadata` is granted — and once granted, the
role reads unredacted values straight off the base table, making the view decorative. **Both halves
of P1b + P2 cannot hold simultaneously with a redact-on-read view.**

**Resolved as option C (2026-09-14, owner: Bikash): redact on write, not on read.** A stored
redacted column (T3c) is what the reporting role is granted; raw `metadata` is never granted to it.
`security_invoker` stays on, so RLS still applies, and the masking no longer depends on the caller
being able to read the thing being masked.

Two alternatives were considered and rejected:

|     | approach                                                                                                         | why not                                                                                                                                                                                      |
| --- | ---------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A   | grant raw `metadata` to the reporting role                                                                       | the redaction becomes decorative — the role can select the raw column directly                                                                                                               |
| B   | drop `security_invoker`, put `WHERE we.tenant_id = current_setting('app.tenant_id', true)::uuid` inside the view | works for Stage 1, but **dies at Stage 2**: `superset-standalone-with-zitadel.md` gives analysts SQL Lab, and any protection that lives inside a view is bypassed by querying the base table |

C is the only option of the three that is still standing after Stage 2 hands analysts a SQL prompt.
Its standing cost is the re-clean path: a `sensitivity` change after rows are stored leaves stale
redactions, which is now a §V invariant and part of T3c rather than something discovered later.
...
| constraint       | value                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BI engine        | Apache Superset `6.1.0` (re-pinned 2026-09-17, D7 — this row drifted stale at `4.0.2` for a while; the running instance was already well past that), tag-pinned like every other third-party image; `superset` and `superset-init` move in lockstep. **Pinning requires a re-pin cadence** — a tracked CVE check against the pinned tag, aligned with how `CLAUDE.md`'s maintenance notes handle npm overrides |
...
| secrets          | no usable defaults in production: the development defaults in `packages/config/src/env.ts` are rejected by a refine when `NODE_ENV=production` (R15). Rotating `SUPERSET_GUEST_TOKEN_SECRET` invalidates live embeds (acceptable at 60s); rotating `SUPERSET_SECRET_KEY` also invalidates Superset's own sessions, so the mint path must retry once on 401                                                     |
...
| data surface     | only what the reporting role is granted, column by column. PII payloads (`workflow_events.metadata`, `entity_instances.fields` and `search_vector`, `tenant_users.email`) are **withheld by grant, not masked by a view** — the masked view was dropped (migration 0119); see ADR-001 and `reporting-metadata-masking-repair.md`. **No tile reads `fields` JSONB**                                             |
| dashboards       | exactly 2, fixed identifiers seeded by provisioning; no picker                                                                                                                                                                                                                                                                                                                                                 |
| deployment       | **Whether Superset starts is configuration, and it starts by default.** `profiles: [reporting]` on the `superset`/`superset-init` services plus `COMPOSE_PROFILES=reporting` in `.env.example`, so a fresh clone comes up with reporting working and a deployment that does not want it unsets one variable. The port is bound to `127.0.0.1` only                                                             |
| deferred         | Celery worker/beat (async chart loading); sync-only for Stage 1 per #102                                                                                                                                                                                                                                                                                                                                       |
| out of scope     | chart authoring, user-written SQL, scheduled/emailed reports, drill-through into Records, dashboard picker. A custom React date picker is explicitly out — the per-KPI-tile date control is a native Superset time filter (D6), not our own component                                                                                                                                                          |
...
│ ╔══════════════════════════ Superset iframe ════════════════╗│
│ ║ ┌────────┐┌────────┐┌────────┐┌────────┐                  ║│  row 1 — 4 native
│ ║ │ Total  ││Overdue ││Unassign││ Median ││ⓘ + own date range ║│  KPI tiles, each
│ ║ │Tickets ││  Now   ││  ed    ││ Time to │ each               ║│  ⓘ + own filter
│ ║ └────────┘└────────┘└────────┘└────────┘                  ║│
│ ║──────────────────────────────────────────────────────────║│
...
Everything inside the double-ruled box is one Superset dashboard, rendered by the iframe — the 4
KPI tiles are no longer a separate React row above it (D6). "Stale (no update 7d+)" and "My
oldest open item" stay out of the native row for now — Stage 1 ships the 4 tenant-tab KPI tiles
that already exist (hidden) in `tiles.yaml`; extending the same pattern to the remaining KPI
tiles across both tabs is a follow-up, not blocking this rework.
...
**Neither identifier is ours to choose, so neither is stored anywhere (decided 2026-09-14, closing
S2).** Both are generated by Superset, which makes any copy we keep — in source, in `.env`, or in a
platform table — a value that can silently drift from the system that owns it. The anchor is
instead the one field we _do_ control:

|                            |                                                                                                                         |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| what we own                | the dashboard **`slug`** — a stable text key set by provisioning: `openwind-tenant-overview`, `openwind-my-performance` |
| what we resolve at runtime | slug → dashboard → its embedded id, asked of Superset on first use and cached                                           |
| what is hardcoded          | **nothing**                                                                                                             |

Three properties this relies on, each verified 2026-09-14 against the running `4.0.2` container
rather than assumed:

1. **`slug` exists and is ours to set** — `Dashboard.__table__` carries `slug` alongside `uuid`,
   `id` and `dashboard_title`. It is independent of the display title, so renaming a dashboard for
   users does not move the anchor.
2. **Re-provisioning does not rotate the embedded id.** `EmbeddedDashboardDAO.upsert()` reuses the
   existing row when one is present — its own docstring: _"Upsert is used to preserve the
   embedded_dashboard uuid across updates."_ This is what makes R6's "identifiers are stable across
   re-runs" true at the source rather than by convention.
3. **The embedded id has no natural key of its own** — `EmbeddedDashboard` carries only `uuid`,
   `dashboard_id` and `allow_domain_list`, so there is nothing on it we could pin. Resolution has to
   start from the dashboard, which is why the slug matters.

Resolution is **lazy — on first mint, never at startup** — so Superset being down cannot stop the
API booting; it degrades to R7's `502 REPORTING_UNAVAILABLE` on the reporting page alone. A `404`
from Superset invalidates the cached entry and re-resolves once, which is what makes a restored or
rebuilt Superset metadata database self-healing: new ids are generated, the slug still finds them,
and nothing needs a redeploy. Caching follows T14's shape (module-level map, TTL on read,
single-flight refresh, invalidate-then-retry-once), which is itself `getCachedOrgTenantId`'s
pattern in `packages/auth/src/middleware.ts` — not a new mechanism.

Two alternatives were rejected, recorded so they are not re-proposed:

| approach                                     | why not                                                                                                                                                                                                                                                               |
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| identifiers in `.env` via `@platform/config` | **chicken-and-egg**: the value does not exist until provisioning has run, so it cannot be set before startup. It would mean start → provision → copy a generated uuid out → paste into `.env` → restart, in every environment, which defeats R16's clone-and-run goal |
| identifiers in a platform database table     | workable, but it makes the platform database authoritative for a value Superset owns. Restore Superset from backup and the table disagrees silently — the same 404 failure as today's hardcoded literals, with no self-heal                                           |
...
What appears on every screen, per role. Written 2026-09-09 by reading a **local prototype**, not
this spec's intent — where the two disagree, both are shown and the divergence is linked to its
open question.

> **Provenance — corrected 2026-09-14. Read this before any row below.**
>
> Every `file:line` in this section refers to an **experimental local branch, not to the
> repository.** Verified against this branch's own HEAD: `apps/admin-ui/src/pages/reporting.tsx`,
> `apps/api/src/routes/reporting/*` (including `guest-token.ts` and `dashboard-ids.ts`),
> `docker/superset/bootstrap.py` and `docker/superset/superset_config.py` **do not exist in the
> repository**; `docker-compose.yml` contains no `superset` service; `packages/config/src/env.ts`
> defines no `SUPERSET_*` variable; and `apps/admin-ui/src/components/layout.tsx`, which does
> exist, contains no Reporting nav entry.
>
> So **nothing in this feature is shipped.** The prototype is exploratory work used to discover
> the design, and this spec — not the prototype — is the source of truth (decided 2026-09-14,
> owner: Bikash, closing OQ-7). Where the prototype does something this spec does not describe,
> the prototype is wrong by definition and is not a constraint on implementation.
>
> The observations below are kept because they are useful design input — they record what was
> learned by building the thing once. They are not a statement of delivered behaviour.

**How to read the status column:**

| marker         | meaning                                                                                                                           |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **prototyped** | present in the local experimental branch, `file:line` given — **not in the repository**                                           |
| **specified**  | described in this doc, not built anywhere — a target                                                                              |
| **assumption** | not verifiable from either source; stated so it can be confirmed or struck                                                        |
| ~~built~~      | **retired 2026-09-14** — it read as "shipped", and nothing in this feature is. Every row that carried it now reads **prototyped** |
...
- **SC-A1 — no chart exists yet (OQ-6).** `bootstrap.py` creates two dashboards and zero `Slice`
  rows. Every tile inventory in §I is therefore **specified**, never **prototyped**. Whatever a user sees
...
| --- | ------------------------------------------ | ------------------------- | ------- | ------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| S1  | Sidebar nav item "Reporting"               | —                         | visible | visible | **hidden**        | **prototyped** — `apps/admin-ui/src/components/layout.tsx:153-160`, in `ADMIN_NAV`; customers return before that array renders |
| S2  | Reporting page shell (heading + tab strip) | `/reporting`              | yes     | yes     | **never reached** | **prototyped** — `apps/admin-ui/src/pages/reporting.tsx`                                                                       |
| S3  | Tab 1 — Tenant Overview (embedded iframe)  | `/reporting`, default tab | yes     | yes     | no                | **prototyped** (frame only, SC-A1)                                                                                             |
| S4  | Tab 2 — My Performance (embedded iframe)   | `/reporting`, second tab  | yes     | yes     | no                | **prototyped** (frame only, SC-A1)                                                                                             |
| S5  | Failure panel                              | `/reporting`              | yes     | yes     | n/a               | **prototyped**                                                                                                                 |
| S6  | Customer redirect                          | `/reporting` → `/records` | n/a     | n/a     | yes               | **prototyped**                                                                                                                 |
...
| Dashboard panel | A single `640px`-tall mount point. The Superset SDK owns everything inside it. Dashboard title is suppressed (`hideTitle: true`) and the filter bar renders collapsed (`filters: { expanded: false }`). Once D6 lands, the filter bar carries 4 additional per-KPI-tile time filters alongside the existing dashboard-wide one — **specified only** until OQ-16 is resolved and provisioned. | `@superset-ui/embedded-sdk`   | **built**                                                    |
...
| Datasets reachable   | the reporting datasets defined in `bootstrap.py`, over the tables the reporting role is granted (`reporting_instances`, `workflow_events`, `workflows`, `workflow_states`, `tenant_users`, `entity_types`) — **built** |
| Tiles present        | 12, provisioned from `docker/superset/tiles.yaml` by `bootstrap.py` — **built**                                                                                                                                        |
| Tiles intended       | §I "Tab 1" lists 11; `tiles.yaml` is what ships and differs from §I in names and count — reconcile §I to it                                                                                                            |
...
|                          |                                                                                                                                                                                                                 |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Superset embedded id     | `f3823557-5d58-4c78-b36f-4d3faeb57013` — **prototyped**                                                                                                                                                         |
| Row filters applied      | Two rules: `tenant_id = '<tenantId>'` **and** `assigned_to = '<userId>'` — **prototyped**, `guest-token.ts`                                                                                                     |
...
(**prototyped**, `guest-token.ts`). This is load-bearing: a Superset RLS clause has no parameterized form.
...
| ------------------------ | --------------------------------- | ---------------------------------------------- | ------------------------ |
| normal                   | tabs + populated charts           | tabs + an empty dashboard frame                | **prototyped** / SC-A1   |
| loading                  | skeleton placeholder in the panel | an empty `640px` div — no spinner, no skeleton | **specified only**       |
| Superset unavailable     | message + retry action            | message, **no retry action**                   | partially **prototyped** |
...
own sentence (**prototyped**, satisfies R7's non-disclosure clause).

### SC-6 — Customer (`user` role): what they see

| Surface                       | Behaviour                                                                                                                                                                                              | Status                                |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- |
| Sidebar nav item              | **Not visible.** The entry lives in `ADMIN_NAV`, and customers return before that array is rendered.                                                                                                   | **prototyped** — `layout.tsx:153-160` |
| `/reporting` (typed directly) | Redirects to `/records` once roles resolve. No tab strip, no iframe, no message explaining the redirect. This is defence-in-depth for direct URL entry, not dead code — the nav never offers the link. | **prototyped** — `reporting.tsx`      |
| Guest-token API               | Refused. `requireRole("agent","admin")` rejects the customer before any mint is attempted.                                                                                                             | **prototyped** — `guest-token.ts`     |
...
and include neither `admin` nor `agent` (**prototyped**, `reporting.tsx`). Note `customer` is not one of
...
One request, end to end. Every hop below is **prototyped** unless the box says otherwise — and
**prototyped means present in the local experimental branch only, never in the repository.** See
§SC's provenance note; the same correction (2026-09-14) applies to every `file:line` in this
section. This diagram records how the prototype was wired, which is useful design input. It is not
a description of shipped behaviour, and it does not constrain the implementation where this spec
says otherwise.
...
**Reading the markers** (added 2026-09-11 after review round 3; **revised 2026-09-14**). A `◻`
means a required property that is **not built yet**, with the task that closes it named inline.
Anything marked `◻` is a merge gate for that task, not an aspiration.

The distinction exists because an earlier revision ticked `✓` against four properties the code does
not have, while the Open Questions table simultaneously listed them as unresolved — a document
asserting and denying the same thing.

**Revision 2026-09-14 — what `✓` may mean here.** The 2026-09-11 pass defined `✓` as "delivered
today", but measured "today" against a **local prototype that is not in the repository** (see
§SC's provenance note — no reporting route, page, Superset service, `SUPERSET_*` config or
`docker/superset/` directory exists in this branch). Nothing in this feature is shipped, so
**almost nothing here can honestly carry `✓`.** Two consequences:

- Wording moved from "not satisfied today" to **"not built yet"**. The first implies working code
  falling short of a requirement; the truth is there is no code.
- **R4's three `✓` marks were corrected to `◻`** — they were the four-properties defect repeating
  itself, missed by the 2026-09-11 sweep because the prototype was being read as shipped state.

A `✓` surviving below asserts something about the **platform as it exists in the repository** —
an existing RLS policy, an existing migration, an existing middleware — never about the prototype.
...
**not built yet**, and the prototype does the opposite: it refuses customers in two places
(`requireRole("agent","admin")` on the mint route, and a `/records` redirect before the page
mounts). Per OQ-7's closure the prototype is wrong, not this requirement. **T19** closes it
◻ a customer cannot reach tenant-wide figures by any route, including a crafted request for the
tenant dashboard — the request is refused server-side, not merely hidden in the UI — **not built
yet.** Note this bullet must survive the bullet above: once customers _are_ admitted to the
My Performance dashboard, the tenant dashboard must still refuse them by role at mint time, which
is a narrower check than today's blanket "no customers at all". **T13** closes it, **T24** proves it
◻ a customer's figures cover only records they are involved in — **not built yet**, and depends on
the assigned-or-created predicate of R3. **T15** (dataset-scoped filters) and **T28** close it

All three carried `✓` until 2026-09-14. That was wrong on the same grounds as R15/R16/R17: the
marker claimed delivered behaviour, and none of the code it rested on is in the repository (see
§SC's provenance note). Corrected rather than quietly dropped, because "a requirement asserting a
property the code does not have" is the exact defect this spec's marker convention exists to
surface.
...
◻ guest permissions land on a dedicated role, never the anonymous one — **not built yet**:
...
◻ pass lifetime is 60s, bounding the window after any disable — **not built yet**:
...
◻ **not built yet** — `packages/config/src/env.ts` gives `SUPERSET_SECRET_KEY`,
...
R16: Superset is not reachable beyond the host, and whether it starts is configuration
◻ **not built yet** — the `superset` and `superset-init` services in `docker-compose.yml`
...
**T4** (loopback bind, mandatory admin password, SQL Lab off), **T6a** (profile key) and **T6b**
(the `.env.example` default, gated on T4 + T5) close
it. Stated as its own requirement (added 2026-09-11) for the same reason as R15

**Amended 2026-09-11 — "does not start unasked" replaced by "whether it starts is
configuration".** The original wording required reporting to be opt-in: a plain `docker compose up`
would not start Superset, and a developer had to pass `--profile reporting` to get it. That is
rejected as a product decision: reporting is a first-class part of the platform, and a fresh clone
must come up with dashboards working rather than requiring a flag nobody discovers.

The two concerns bundled in the original wording are separable, and only one of them was ever a
security property:

- **network exposure and default credentials** — a genuine security requirement, unchanged and
  still `◻`. Loopback bind, no default secrets (R15), dedicated guest role (R6), SQL Lab off.
- **whether the container starts by default** — a developer-experience and resource question, not
  a security one. A loopback-bound Superset with no default credentials is not more dangerous for
  having started.

**Read the baseline before judging this amendment as an increase in exposure — it is a reduction.**
With no `profiles:` key on the services, **Superset already starts on every `docker compose up -d`
today**, and there is no way to stop it short of editing the compose file. So this amendment does
not move from "off by default" to "on by default"; it moves from _"always starts, no control"_ to
_"starts by default, one variable turns it off"_. The control is what is new. (Added 2026-09-14 —
the amendment read as though it were opening something that is in fact already open.)

The resolution keeps the `profiles:` key (so starting it stays controllable and a deployment that
does not want reporting can switch it off without editing compose files) and ships
`COMPOSE_PROFILES=reporting` in `.env.example`. A fresh clone therefore starts Superset, and
turning it off is one variable, not a compose edit. This is why the requirement is phrased as
"whether it starts is configuration" rather than either "always starts" or "never starts by
default" — neither absolute belongs in the compose file.

R17: no reporting identifier or endpoint is hardcoded in application source
◻ **not built yet** — `apps/api/src/routes/reporting/dashboard-ids.ts` carries four literal
dashboard UUIDs in TypeScript. Beyond being configuration in the wrong place, it is **broken on a
fresh clone**: provisioning generates its own identifiers, which will not match the literals, so
the mint succeeds and every dashboard API call behind it 404s (the §B B10 failure mode, reached by
a different route). **Corrected 2026-09-14 (S2):** this requirement previously said provisioning
and the API must read the identifiers "from the same configuration source", which cannot be done —
the identifiers do not exist until provisioning has run, so no pre-startup configuration can hold
them. The requirement is therefore that **no generated identifier is stored anywhere**: provisioning
and the API agree on a `slug` we own, and the identifiers are resolved from Superset at runtime and
cached. A stored copy of a value another system generates is drift waiting to happen, in `.env` and
in a platform table alike. See §I "identifiers". **T12b** closes it
...
| I      | anyone on the network reaches Superset and uses SQL Lab as `admin`                                                                                                            | **NOT BLOCKED TODAY** — P3 is open in code (R16). Blocked by **T4 + T6a**, phase 0              |
...
- no identifier generated by Superset is stored in source, in `.env` or in a platform table —
  provisioning and the API agree on a slug we own, and identifiers are resolved at runtime and
  cached (S2, 2026-09-14). A stored copy of a value another system owns drifts from it silently
...
- views must be `security_invoker` so RLS actually applies through them, not just to direct table
  access (P1b)
- **the reporting role never holds a grant on raw `workflow_events.metadata`** — it reads the
  redacted stored column instead (OQ-13 option C, 2026-09-14). This is the invariant that makes the
  masking real rather than decorative: a protection that only exists inside a view is bypassed by
  anyone who queries the base table, and Stage 2 hands analysts exactly that ability
- **redaction must not go stale** — if a field's `sensitivity` changes after rows are stored, the
  stored redacted copies are re-computed. A redaction that is only correct at insert time silently
  un-masks the field it was added to hide
...
| T2  | implement the chosen isolation model                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | T1         | TBD                                                  |
| T3  | re-assert migration 0009's grant allowlist + a test pinning the granted set (P2) — generate the concrete list from the live grants, not hand-typed, and commit it into the spec/migration so the pinning test has an exact set to diff against. **Count re-verified 2026-09-11: still 34.** Migrations `0092`–`0103` (PRs #583/#585/#586: teams, services, on-call schedules, labels, ticket*labels, notification_policies, schedule_rules, schedule_executions) were each read directly — **none grants `analytics_user`**, which is migration `0009`'s `ALTER DEFAULT PRIVILEGES ... REVOKE` working as intended: new tables are no-access unless a migration explicitly grants them. That default-deny is why the baseline held; the pinning test exists to catch a grant made **outside** a migration, which is how the P2 drift happened. \*\*Amended 2026-09-14 (OQ-13 → option C): the test must pin the granted \_columns*, not only the granted tables.\*\* Under option C the whole protection is that `analytics_user` holds the redacted column and never raw `workflow_events.metadata` — a table-level pin would not notice that single column being re-granted, which is precisely the failure it exists to catch | —          | TBD — **GH issue to be filed before phase 0 starts** |
| T3b | **set `security_invoker = true` on `workflow_events_masked` and any other reporting view, in the SAME migration as T3** — with `security_invoker` on, the view evaluates as the **caller**, so the caller needs its own read grants on whatever the view selects (P1b). **Under option C that grant is the redacted column from T3c, never raw `metadata`.** Why this ordering is load-bearing, verified live 2026-09-14 against `ow-database` in rolled-back transactions: with `security_invoker = true` and raw `metadata` withheld, `SELECT count(*) FROM workflow_events_masked` fails with `ERROR: permission denied for table workflow_events` — the view reads `we.metadata` internally to redact it. Adding raw `metadata` to the grant makes the view work, but then the role can read unredacted values straight off the base table and the masking is decorative. T3c is what resolves the conflict; this task must not land before it                                                                                                                                                                                                                                                                               | T3, T3c    | TBD                                                  |
| T3c | **redacted stored column for `workflow_events.metadata` — OQ-13 resolved 2026-09-14 as option C.** Add a stored, pre-redacted copy of `metadata` (working name `metadata_masked`), keep it in sync on write, and backfill existing rows. `analytics_user` is granted the redacted column and **never** the raw one, so the protection holds even where a view is bypassed. Three things the implementation must not rediscover the hard way: (1) **a generated column cannot do this** — verified 2026-09-14: `ERROR: cannot use subquery in column generation expression`, because the redaction must consult `entity_fields.sensitivity`, which lives in a different row; so it is a trigger or write-path mechanism, not `GENERATED ALWAYS AS`; (2) **a re-clean path is mandatory** — if a field's `sensitivity` is changed to `pii`/`financial` after rows are stored, previously redacted copies are stale and silently expose what the change was meant to hide, so changing sensitivity must trigger a re-redaction of affected rows, and that path needs its own test; (3) redaction cost moves from read time to write time, which is the intended trade but should be stated rather than discovered                   | —          | TBD                                                  |
| T4  | loopback bind, mandatory admin password, SQL Lab off (P3). **SQL Lab off is a Stage 1 posture, not a permanent one** — `superset-standalone-with-zitadel.md` deliberately re-enables it for analysts, on a connection that satisfies P1 first                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | —          | TBD                                                  |
...
| T6a | `profiles: [reporting]` on the `superset` / `superset-init` services. This half only _adds a control that does not exist today_ — with no `profiles:` key the services already start on every `docker compose up -d`, so nothing becomes more exposed by making that behaviour switchable (R16). Also digest-pin the images **and record a re-pin/CVE-check cadence**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | —          | TBD                                                  |
| T6b | **`COMPOSE_PROFILES=reporting` in `.env.example`**, so a fresh clone comes up with reporting working and switching it off is one variable rather than a compose edit (R16, amended 2026-09-11). **Split from T6a and gated on T4 + T5 (2026-09-14).** This is the half that decides what a fresh clone actually runs, and R16's safety argument — _"a loopback-bound Superset with no default credentials is not more dangerous for having started"_ — is only true once T4 and T5 have landed. Shipped before them, this line turns an exposed instance (all interfaces, default admin password, SQL Lab on, working default `SUPERSET_SECRET_KEY`) into the documented default developer setup                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | **T4, T5** | TBD                                                  |
...
**The `depends` column was added 2026-09-14.** Phase 0 previously had none — only phase 1 did — so ordering between phase 0 tasks was expressed, when at all, as prose inside a task cell (T3b's "in the SAME migration as T3"). That is how T6 came to carry no recorded dependency on T4/T5 while being amended to start Superset by default, and how T3b's dependency on T3 stayed invisible. Ordering that matters for safety belongs in a column the reader cannot skim past.

### phase 1 — the mechanism

| id   | task                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | depends                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T11  | Superset service, own metadata database, cache index, config                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | T4,T6a                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| T12  | provisioning: data connection, two dashboards, embeddability, dedicated guest role — **must also remove `GUEST_ROLE_NAME = "Public"` and point it at the new dedicated role; the prototype's `bootstrap.py` grants the six embedded-viewer permissions to the anonymous role (R6, OQ-11 closed 2026-09-11)**. **Amended 2026-09-14 (OQ-13):** provisioning must register **`workflow_events_masked`, never raw `workflow_events`** — migration `0009` excludes the raw table from the reporting grant because `metadata` may carry PII, and the prototype re-exposes it by registering the physical table directly. Also set **`expose_in_sqllab=False`** on the connection for Stage 1. **`entity_instances` stays registered as-is** — `0009:77` already grants it an explicit column allowlist that omits `fields`, the only PII-bearing column on it, and there is no `entity_instances_masked` view in the repository to point at (checked: `workflow_events_masked` is the only masked view that exists). Do this before any chart is authored — no `Slice` exists yet, so the swap is free now and a rewrite of every `workflow_events`-backed tile later | T2,T11,**T3b**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| T12b | **remove the hardcoded dashboard identifiers (R17); resolve them by slug instead (S2 answered 2026-09-14).** The prototype's `apps/api/src/routes/reporting/dashboard-ids.ts` holds four literal UUIDs while provisioning generates its own, so a fresh clone mints a valid pass whose every dashboard call 404s. **The fix is not to store the generated ids somewhere else — it is to stop storing them at all.** Provisioning sets a `slug` we own (`openwind-tenant-overview`, `openwind-my-performance`); the mint path resolves slug → dashboard → embedded id from Superset on first use and caches it, with no literal in source, no `.env` entry and no platform table. Cache follows T14's shape (TTL, single-flight, invalidate-then-retry-once); resolution is **lazy, never at startup**, so a Superset outage degrades to R7's 502 on the reporting page instead of blocking API boot; a 404 invalidates and re-resolves once, which makes a restored Superset metadata database self-healing. See §I "identifiers" for the three verified container facts this rests on and for why `@platform/config` and a platform table were both rejected    | T12                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| T25d | telemetry: `reporting_mint_total`/`reporting_mint_duration_seconds`/`reporting_session_cache_hit_ratio`/`reporting_active_tenants_total` metrics; tenant/dashboard/cache-hit attributes on the active OTel span in `guest-token.ts`; the three new alert rules; the INFO/WARN log additions (see §C telemetry)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | T13,T14 **Mint-path span attributes made explicit (S3 answered 2026-09-14):** `reporting.tenant_id`, `reporting.user_id`, `reporting.dashboard_id` and `reporting.filter_count` are **mandatory** on every mint, attached to the active span via `trace.getActiveSpan()?.setAttributes(...)` at the point in the mint handler where they are known — never as a hand-rolled span, per this repo's existing practice of auto-instrumentation only. Rationale: a wrong filter on this path is a live cross-tenant leak, and T20's audit log is the durable record but not the real-time one — these attributes are what an on-call engineer reads while the anomaly is still happening. `filter_count` specifically catches R5's failure mode (a mint that went out with fewer filters than datasets) without needing the clause contents, which must never reach telemetry. |
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OQ-6  | ~~**Neither dashboard contains any chart. What is the intended tile inventory, and who builds it?**~~ **CLOSED 2026-09-14 — the question rested on reading the prototype as shipped state.** The inventory is not unknown: §I specifies 11 Tenant Overview tiles and 6 My Performance tiles, and **T27**/**T28** build them against seeded demo data (**T26**), gated on **T15**. That the prototype's `bootstrap.py` creates two dashboards and zero `Slice` rows is not a gap in this spec — it is a prototype that stopped before the tiles, and per OQ-7's closure the prototype does not constrain the build. **Kept as a live risk, not a question:** SC-A1 still assumes a chartless dashboard renders as an empty frame rather than an error tile, which is unverified and changes T27/T28's priority if wrong.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Evidence unchanged from the 2026-09-09 reading; note per §SC that it describes the local prototype, not the repository.                                                                                                                                                                                                                                                                      |
| OQ-7  | ~~**Do customers get reporting at all?**~~ **CLOSED 2026-09-14 — decided, owner: Bikash. The docs lead; the code is an experiment.** §D4 and §R4 stand: a customer gets a single My Performance tab, scoped to records they are involved in, with no tab strip rendered. The prototype refuses customers in two places (`requireRole("agent","admin")` on the mint route, and a `/records` redirect before the page mounts) — that is the prototype being **behind this spec, not superseding it**. Two consequences recorded rather than left implicit: **(1)** R4's three bullets were carrying `✓` and are now `◻` with closing tasks named, because none of the code they rested on is in the repository; **(2)** the narrower rule survives — admitting customers to My Performance must not admit them to the tenant dashboard, which stays refused server-side by role at mint time (R4, **T13**, proven by **T24**).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Evidence unchanged from the 2026-09-09 reading; note per §SC that it describes the local prototype, not the repository.                                                                                                                                                                                                                                                                      |
...
| OQ-9  | ~~**The user dashboard's filter names a column one of its two datasets does not have.**~~ **CLOSED 2026-09-14 — folded into R5/T15, and one of the three options it offered was not actually available.** `assigned_to` exists on `entity_instances` (`packages/db/src/schema/entity-engine.ts:77`) but not on `workflow_events` (`packages/db/src/schema/workflow-engine.ts:113-154`), and the prototype's `buildRlsRules` emits `{ clause }` with no `dataset` key, so Superset would apply it to every dataset. **The fix is dataset-scoped rules — R5 already requires exactly this, and T15 already implements it**; the clause is attached only to datasets that declare the column, and a dataset no filter covers refuses the request rather than serving it unfiltered. **The option this OQ listed as "a view exposing `assigned_to`" is not available as written:** verified 2026-09-14, `workflow_events_masked` does not expose `assigned_to` — it selects only `we.*` plus the rebuilt metadata and joins `entity_instances` solely to resolve `ei.entity_type_id` for the redaction lookup, projecting no `ei` column out. Taking that route would mean authoring a **new** view, not reusing the existing one. Since OQ-13 routes all `workflow_events` reads through the masked path, this is settled by T15 rather than by widening the view.                                                                                                                                                                                                                                                            | Evidence unchanged from the 2026-09-09 reading; note per §SC that it describes the local prototype, not the repository.                                                                                                                                                                                                                                                                      |
...
| OQ-12 | ~~**Superset starts by default and binds all interfaces.**~~ **CLOSED 2026-09-11 — no decision was ever needed, only an honest statement.** The gap is real and stays open in code; it is now stated as requirement **R16** with `◻`, the §C deployment row is marked target-not-current, and the §S "anyone on the network reaches Superset" row reads NOT BLOCKED TODAY. Closed by **T4 + T6a**, phase 0.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `docker-compose.yml` — `superset:` and `superset-init:` blocks; compare `ow-backend`'s `"127.0.0.1:${API_HOST_PORT:-3002}:3000"`.                                                                                                                                                                                                                                                            |
| OQ-13 | ~~**The datasets are the raw physical tables, not masked views.**~~ **CLOSED 2026-09-14 — resolved as option C (pre-computed redacted column), owner: Bikash.** The prototype registers `entity_instances` and `workflow_events` as raw physical tables with `expose_in_sqllab=True`, while migration `0009` excludes raw `workflow_events` from the reporting grant precisely because `metadata` may carry PII. **T12** now registers `workflow_events_masked` instead and sets `expose_in_sqllab=False`; `entity_instances` stays as-is, already column-allowlisted at `0009:77` with `fields` omitted, and no `entity_instances_masked` exists to point at. **The OQ as originally written asked the wrong question** — it asked _when_ P1-A lands relative to chart authoring, a sequencing question, when the blocker was mechanism. Verified live 2026-09-14 in rolled-back transactions: `security_invoker = true` (T3b) makes the view evaluate as the caller, so the caller needs a grant on `metadata` — the very column being hidden. Without it `SELECT count(*) FROM workflow_events_masked` fails `permission denied for table workflow_events`; with it, the role reads unredacted values straight off the base table. **Option C breaks the deadlock**: a stored redacted column the role is granted, raw `metadata` never granted (**T3c**). C was chosen over the alternatives because it is **the only one that survives Stage 2** — `superset-standalone-with-zitadel.md` gives analysts SQL Lab, and any protection living inside a view is bypassed by an analyst who simply queries the base table. | Evidence unchanged from the 2026-09-09 reading; note per §SC that it describes the local prototype, not the repository.                                                                                                                                                                                                                                                                      |
...
| OQ-16 | **No dashboard in this repo has ever scoped a native filter to a single chart.** D6 needs 4 independent time filters, each via `chartsInScope` naming exactly one KPI tile — every filter provisioned so far (`tiles.yaml`'s `Date Range` entries) is one filter scoped to every chart on the dashboard, computed in `bootstrap.py`'s `filter_time` branch as "all charts on this dashboard key". Scoping to one chart means that branch needs to accept an explicit chart list instead of always defaulting to "all" — mechanically supported by the `NATIVE_FILTER`/`scope`/`chartsInScope` shape already written, but unexercised. Flagged so a reviewer signs off on the first instance of this pattern, not so it can be discovered later as an unreviewed first.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `docker/superset/bootstrap.py` `apply_filters`/`filter_time` (~lines 1016-1026); `tiles.yaml:754-761`.                                                                                                                                                                                                                                                                                       |

## §B Bugs / Backprop Log

| id  | what failed                                                                                                                                                                                                                                                                                                                                     | root cause                                                                                                                                                                                                                                                                                                                                                                                                    | promoted to §V?                                                                                                                                     |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| B11 | **Superset's REST API answers unauthenticated callers.** Observed 2026-09-11 against the running prototype container: `GET /api/v1/dashboard/`, `/api/v1/chart/` and `/api/v1/dataset/` each returned **HTTP 200 with no credentials** — not `401`. Zero rows came back, so this was not a live data leak, but the authorization gate was open. | `AUTH_ROLE_PUBLIC = "Public"` in Flask-AppBuilder assigns the `Public` role to **unauthenticated** users, and provisioning grants the six embedded-viewer permissions to that same role. So the permissions intended for embedded guests were handed to anonymous callers. Compounds with R16: this gate is only reachable because the instance binds every interface — the two are not independent findings. | yes — "guest permissions never land on the anonymous role" (already in §V); closed by **T12** (dedicated guest role, R6) and **T4** (loopback bind) |
| B12 | **A redact-on-read view cannot be read by the role it protects.** `workflow_events_masked` with `security_invoker = true` fails `permission denied for table workflow_events` for the reporting role, because the view reads the raw `metadata` column the P2 grant deliberately withholds.                                                     | The P1b fix (`security_invoker`) and the P2 grant allowlist were each correct in isolation and were never tested together. `security_invoker` moves the permission check to the caller, so the caller needs access to the column being masked — a circular requirement. See §P1c.                                                                                                                             | yes — "the reporting role never holds a grant on raw `workflow_events.metadata`"; resolved by **T3c** (option C, redact on write)                   |
