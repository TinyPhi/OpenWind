...
| data surface     | only what the reporting role is granted; masked reads per ADR-001 — **and after OQ-13's option C (2026-09-14) the masking is a granted-column boundary, not only a view**: `analytics_user` holds the redacted copy of `workflow_events.metadata` and never the raw column, so the protection survives a query that bypasses the view entirely. **No tile reads `fields` JSONB**                                                                                                                                                                                                                                                                                                                                                    |
| dashboards       | exactly 2, fixed identifiers seeded by provisioning; no picker                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| deployment       | **Whether Superset starts is configuration, and it starts by default.** `profiles: [reporting]` on the `superset`/`superset-init` services (T6a) plus `COMPOSE_PROFILES=reporting` in `.env.example` (T6b), so a fresh clone comes up with reporting working and a deployment that does not want it unsets one variable. **Corrected 2026-09-14:** this row previously read "an opt-in compose profile — a plain `docker compose up` does not start it", which is the exact framing R16's 2026-09-11 amendment rejects; the two contradicted each other inside one document. **Not built yet:** no `profiles:` key exists and, per §SC's provenance note, no `superset` service exists in `docker-compose.yml` at all (R16/T6a/T6b) |
...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ----------------------------------------------------------------- |
| Page heading    | Static text "Reporting"                                                                                                                                                                                 | hardcoded                     | **prototyped** — `reporting.tsx`                                  |
| Tab strip       | Exactly two pill buttons, "Tenant Overview" and "My Performance". Both always rendered for anyone who reaches the page. Active tab styled; no counts, no badges.                                        | hardcoded `["tenant","user"]` | **prototyped**                                                    |
| Dashboard panel | A single `640px`-tall mount point. The Superset SDK owns everything inside it. Dashboard title is suppressed (`hideTitle: true`) and the filter bar renders collapsed (`filters: { expanded: false }`). | `@superset-ui/embedded-sdk`   | **prototyped**                                                    |
| Failure panel   | Replaces the panel entirely with one sentence: _"Reporting is not available right now. Try again shortly."_ No retry button — the user must switch tabs or reload.                                      | `reporting.tsx` catch block   | **prototyped**; §I promises "a retry action" — **specified only** |
...
|                      |                                                                                                                                                     |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Superset embedded id | `f3823557-5d58-4c78-b36f-4d3faeb57012` — **prototyped**, `apps/api/src/routes/reporting/dashboard-ids.ts`                                           |
| Row filter applied   | `tenant_id = '<caller's tenantId from JWT>'` — **prototyped**, `guest-token.ts` `buildRlsRules`                                                     |
| Filter scope         | **Unscoped by dataset** — the rule carries no `dataset` key, so Superset applies it to every dataset on the dashboard. **prototyped**, and see OQ-9 |
| Datasets reachable   | `entity_instances`, `workflow_events` — **prototyped**, `bootstrap.py`                                                                              |
