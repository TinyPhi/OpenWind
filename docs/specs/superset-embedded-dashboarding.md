...
| T3  | re-assert migration 0009's grant allowlist + a test pinning the granted table set (P2) — generate the concrete table list from the live grants (confirmed 34 tables), not hand-typed, and commit it into the spec/migration so the pinning test has an exact set to diff against | TBD                                 |
...
| OQ-10 | **What is the actual guest-token lifetime?** §I states 60 seconds. `GUEST_TOKEN_JWT_EXP_SECONDS` is **not set** in `docker/superset/superset_config.py`, so the value is whatever Superset 4.0.2 defaults to. R9's revocation window is stated against 60s and is currently unverified.                                                                                                                                                                                                                                                                                                                 | `docker/superset/superset_config.py` — sets `GUEST_TOKEN_JWT_SECRET` and `GUEST_TOKEN_JWT_ALGO`, no exp setting.                                                                                                                                                                                                                                                                             |
| OQ-11 | **Guest permissions are on `Public`.** §R6 requires a dedicated role, "never Flask-AppBuilder's anonymous `Public` role". `superset_config.py` sets `GUEST_ROLE_NAME = "Public"` and `bootstrap.py` grants the six embedded-viewer permissions to exactly that role. Is R6 still the target?                                                                                                                                                                                                                                                                                                            | `superset_config.py:36`; `bootstrap.py` `guest_role_name = current_app.config.get("GUEST_ROLE_NAME", "Public")`.                                                                                                                                                                                                                                                                             |
| OQ-12 | **Superset starts by default and binds all interfaces.** §C calls it an opt-in compose profile; the `superset` and `superset-init` services carry **no `profiles:` key**, and the port mapping is `"${SUPERSET_HOST_PORT:-8088}:8088"` — no `127.0.0.1` prefix, unlike backend/postgres/redis. P3 is therefore still open in code.                                                                                                                                                                                                                                                                      | `docker-compose.yml` — `superset:` and `superset-init:` blocks; compare `ow-backend`'s `"127.0.0.1:${API_HOST_PORT:-3002}:3000"`.                                                                                                                                                                                                                                                            |
