...
| T2  | Mint endpoint: validate required fields + Client ID uniqueness (including reclaiming an expired-but-not-yet-revoked key's Client ID, since the partial index alone can't exclude that case) + non-empty `scopes` against known vocabulary, stamp `expires_at` = now + 3mo                                                                                                                       | 1     | in-progress (PR A2) | T1          |
