...
| ----- | ---------------------------------------------------------------- | ------------------------------------------- | ----------------------------------------------- |
| 1     | embedded in admin-ui `/reporting`, no Superset login             | all roles, tabs gated per role              | `superset-embedded-dashboarding.md` — adopted   |
| 2     | Superset's own URL, Zitadel login, user writes their own queries | analysts who need to explore, not just view | **built** — running locally, see the note below |

### Status correction (2026-09-22)

This document recorded Stage 2 as deferred with every task `todo` from 2026-09-18 onward, while
the work was in fact built and serving logins. That gap is itself worth recording, because it is
why a grant regression went unnoticed for two migrations.

What is actually in place, verified against the running instance rather than assumed:

- `AUTH_OAUTH` against Zitadel, roles synced at login, tenant bound as a `tenant:<uuid>` role and
  the own-rows subject as `owuser:<subject>` (T5, T6 — `docker/superset/superset_config.py`).
- `ReportingAnalyst` provisioned with SQL Lab menu, REST and `database_access` permissions, and
  the reporting connection exposed in SQL Lab when a client id is configured
  (`docker/superset/bootstrap.py`).
- Two real accounts have logged in and carry the expected roles. Tenant and own-rows scoping were
  measured end to end: staff saw the whole tenant, non-staff saw only their own rows.

What is **not** in place, and still blocks a real deployment: export is deliberately ungranted
pending T12, and T9 through T17 below remain open. The phase gate at the bottom of §T was passed
through rather than met — see `reporting-metadata-masking-repair.md` §B5.

Stage 1 (embedded) remains the adopted production path for dashboards.
...
Stage 2 is off until `SUPERSET_OAUTH_CLIENT_ID` is set; the embedded dashboards work without any
of this.

1. Run `pnpm setup:superset-oauth`. It creates (or updates) Superset's Zitadel app and prints
   `SUPERSET_OAUTH_CLIENT_ID` and `SUPERSET_OAUTH_CLIENT_SECRET`. Put both in `.env.local` — never
   in `.env`, which is committed.
2. Set `SUPERSET_OAUTH_INTERNAL_ISSUER` in `.env.local`. This is how the Superset container reaches
   Zitadel, and it must be a domain your Zitadel instance answers to: Zitadel picks the instance
   from the `Host` header, so the bare container name returns 404. Use the instance's generated
   domain, e.g. `http://<instance-domain>:8080`. A domain is right when
   `curl -H "Host: <instance-domain>:8080" http://localhost:8080/.well-known/openid-configuration`
   returns 200.
3. Make that name resolve on the Docker network Superset shares with Zitadel, by giving the Zitadel
   container a network alias:
   `docker network connect --alias <instance-domain> <network> <zitadel-container>`
   (disconnect first if it is already attached).
4. Recreate Superset: `docker compose up -d --force-recreate superset superset-init`.

A `Host`-header override on the container name was tried instead of step 3 and is not a
substitute: authlib's metadata-discovery request does not apply it.
...
| T5  | Zitadel OIDC client + `AUTH_OAUTH` with roles sync at login                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 1     | **built** (2026-09-22)                                                                                                                                                                                                                                                                                                                                                    | T2         |
| T6  | claim→role/tenant mapping via `lookupTenantIdByOrgId`; no-access default; refuse tenant-less                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 1     | **built** (2026-09-22)                                                                                                                                                                                                                                                                                                                                                    | T5         |
...
| T9  | session max duration + idle timeout with **stated values**; propagate Zitadel single-logout (P5, R4)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 1     | **partial** — session max set, no SLO                                                                                                                                                                                                                                                                                                                                     | T5         |
| T10 | require MFA for the reporting-analyst role at the Zitadel level (R6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 1     | todo                                                                                                                                                                                                                                                                                                                                                                      | T7         |
| T11 | query/export audit trail keyed to the Zitadel subject, recording **what** was exported, shipped to the platform's append-only store (R7)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 2     | **done** — `PlatformAuditEventLogger` (docker/superset/superset_config.py) writes every SQL Lab query and export to `admin_audit_log` through `record_reporting_audit()` (migration 0115), with the query text, row count and duration, bound to the session tenant. Gap: the actor is the Zitadel login name (falling back to the subject id), not always the subject id | T6         |
