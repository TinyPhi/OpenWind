...
| query/export audit records (R9)          | `writeAuditEntry` from `@platform/audit` into the append-only `admin_audit_log`; Superset's own logs are not an audit trail | `apps/api/src/routes/api-keys/create.ts`; migration 0011                    |
| export rate limiting (R9)                | ADR-013's tier shape, same as Stage 1's mint limit — a per-tenant aggregate tier, not a new bespoke scheme                  | `docs/decisions/ADR-013-unified-rate-limiting-strategy.md`                  |
...
R8: the reporting-analyst role requires step-up authentication
...
R9: exporting is bounded and attributable
...
| S      | an analyst's Zitadel role is revoked but their Superset session keeps working       | P5 — session lifetime, idle timeout and single-logout, all with stated values        |
...
| T10 | require MFA for the reporting-analyst role at the Zitadel level (R8)                                                                                                                                                     | 1     | todo                                | T7         |
| T11 | query/export audit trail keyed to the Zitadel subject, recording **what** was exported, shipped to the platform's append-only store (R9)                                                                                 | 2     | todo                                | T6         |
| T12 | export limits distinct from query limits — bulk/repeated export rate limiting (R9)                                                                                                                                       | 2     | todo                                | T6         |
| T13 | resource limits: row cap, query timeout, and **revisit the read-replica decision** — Stage 1 deliberately reads the live DB (users can't write queries there); Stage 2 lets analysts run heavy ad-hoc queries against it | 2     | todo                                | T2         |
...
| T15 | retention/erasure coverage for Superset's Redis result cache and export volume — tenant data lives outside the masked views                                                                                              | 2     | todo                                | T6         |
...
| T17 | abuse-case tests from §S — cross-tenant attempts return zero rows                                                                                                                                                        | 2     | todo                                | T6         |
...
| OQ-5 | Is content-aware export auditing plus an export-specific rate limit the right scope, or is more DLP expected? | R9. Row caps and query timeouts bound _query_ cost, not exfiltration volume via repeated exports.                                                                                                                                                                                                                                                                                                                                                       |
