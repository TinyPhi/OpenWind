...
| T20: Isolation test — rotate a key after a ticket is tagged; confirm the tag still resolves to the same application afterward. Blocked on T9 (no read-side resolution to assert against yet)                                                                                                            | R7          | todo   |
| T21: Isolation test — rename an app's `applicationName` after tickets are tagged; confirm those tags now display the new name, not a frozen one. Blocked on T9                                                                                                                                          | R7          | todo   |
