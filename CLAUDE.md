...
documented in this section. #127 closed via PR #155 (2026-07-21).
...
- [x] [#127](../../issues/127) `setEntityState` / `bulkSetState` are unguarded state side-doors: mutate `current_state` directly with no `workflow_events` row and no outbox event. — ✅ Done, PR #155 (2026-07-21); both functions now write a `workflow_events` row + `workflow.transitioned` outbox event in the same transaction.
